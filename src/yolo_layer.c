...
layer make_yolo_layer(int batch, int w, int h, int n, int total, int *mask, int classes, int max_boxes)
{
    ...
    l.biases = (float*)xcalloc(total * 3, sizeof(float));// 2--> w,h  3--> w h theta
    if(mask) l.mask = mask;
    else
    {
    ...
    l.bias_updates = (float*)xcalloc(n * 3, sizeof(float));
    l.outputs = h*w*n*(classes + 4 + 1+1);
    l.inputs = l.outputs;
    l.max_boxes = max_boxes;
    l.truth_size = 4 + 2+1;// x y w h theta obj class
    ...
    l.delta = (float*)xcalloc(batch * l.outputs, sizeof(float)); // fanxiangchuangbo wuchadaoshu
    l.output = (float*)xcalloc(batch * l.outputs, sizeof(float));// shuchujieguo
    for(i = 0; i < total*3; ++i)
    {
        l.biases[i] = .5;
    }
...
void resize_yolo_layer(layer *l, int w, int h)
{
    ...
    if (l->labels) l->labels = (int*)xrealloc(l->labels, l->batch * l->n * l->h * l->w* l->theta * sizeof(int));
...
ious delta_yolo_box(box truth, float *x, float *biases, int n, int index, int i, int j, int lw, int lh, int w, int h, float *delta, float scale, int stride, float iou_normalizer, IOU_LOSS iou_loss, int accumulate, float max_delta, int *rewritten_bbox)
{
    ...
    if (iou_loss == MSE)    // old loss
    {
        ...
        delta[index + 4 * stride] += ttheta - pred.theta;
        // add theta MSE
    ...
    else 
    {
        ...
        // printf("******************** NEW LOSS ****************************\n");
        // printf("----------------  truth ------------------\n");
        // printf("%f  %f  %f  %f  %f\n",truth.x,truth.y,truth.w,truth.h,truth.theta);
        all_ious.dx_iou = dx_box_iou(pred, truth, iou_loss); //求各IOU的梯度
        ...
        float dtheta = all_ious.dx_iou.dtheta;
...
static int entry_index(layer l, int batch, int location, int entry)
{
    ...
    return batch*l.outputs + n*l.w*l.h*(5+l.classes+1) + entry*l.w*l.h + loc; // yuanlaiwei 4 xiugaiwei 5 buzhidaoduibudui
}

void forward_yolo_layer(const layer l, network_state state)
{
    // printf("forward yolo layer*********\n");
    int i, j, b, t, n;
    ...
    for (b = 0; b < l.batch; ++b) 
    {
        for (n = 0; n < l.n; ++n) 
        {
            ...
            index = entry_index(l, b, n*l.w*l.h, 5); // 原来的值为4
            activate_array(l.output + index, (1 + l.classes)*l.w*l.h, LOGISTIC);
        }
    }
#endif
    
    // delta is zeroed init l.delta, l.delta is gradient
    ...
    *(l.cost) = 0;
    ...
    for (b = 0; b < l.batch; ++b) 
    {
        for (j = 0; j < l.h; ++j) 
        {
            for (i = 0; i < l.w; ++i) 
            {
                // printf("------------ l.h:  %d  l.h:  %d  l.n:  %d\n",l.h,l.w,l.n);
                for (n = 0; n < l.n; ++n) 
                {
                    const int class_index = entry_index(l, b, n*l.w*l.h + j*l.w + i, 6); // classfication 的入口
                    const int obj_index = entry_index(l, b, n*l.w*l.h + j*l.w + i, 5); // obj 的入口
                    const int box_index = entry_index(l, b, n*l.w*l.h + j*l.w + i, 0); // box x y w h theta 的入口 猜的
                    ...
                    {
                        box truth = float_to_box_stride(state.truth + t*l.truth_size + b*l.truths, 1);// 某一个anchor内的truth
                        if (!truth.x) break;  // continue;
                        int class_id = state.truth[t*l.truth_size + b*l.truths + 5]; //找到class_id
                        ...
                        float objectness = l.output[obj_index]; //找到 有物体置信度值
                        ...
                        float iou = box_iou(pred, truth); // 计算IOU  xiekuangjisuanshi zhelixuyaoxiugai
                    ...
                    avg_anyobj += l.output[obj_index]; // 有物体的平均概率
                    ...
                    else if (state.net.adversarial) 
                    {
                        ...
                        if (found_object) // 找到物体
                        {
                            ...
                            l.delta[box_index + 4 * stride] += scale * (0 - l.output[box_index + 4 * stride]); // re-considering what is delta? delta is error
                            printf("l.delta[%d]:  %f, l.delta[%d]:  %f, l.delta[%d]:  %f, l.delta[%d]:  %f, l.delta[%d]:  %f\n",box_index + 0 * stride,l.delta[box_index + 0 * stride],box_index + 1 * stride,l.delta[box_index + 1 * stride],box_index + 2 * stride,l.delta[box_index + 2 * stride],box_index + 3 * stride,l.delta[box_index + 3 * stride],box_index + 4 * stride,l.delta[box_index + 4 * stride]);
                        }
                    }
                    if (best_iou > l.truth_thresh) 
                    {
                        ...
                        int class_id = state.truth[best_t*l.truth_size + b*l.truths + 5];
        ...
        for (t = 0; t < l.max_boxes; ++t) 
        {
            ...
            int class_id = state.truth[t*l.truth_size + b*l.truths + 5];
            ...
            for (n = 0; n < l.total; ++n) 
            {
                ...
                {
                    best_iou = iou;
                    best_n = n; // 标记IOU最符合的那个序号
                }
            }

            int mask_n = int_index(l.mask, best_n, l.n);// 找到anchor框中最匹配的那一个
            if (mask_n >= 0) // 最匹配的序号
            {
                int class_id = state.truth[t*l.truth_size + b*l.truths + 5];
                if (l.map) class_id = l.map[class_id];
                ...
                const float class_multiplier = (l.classes_multipliers) ? l.classes_multipliers[class_id] : 1.0f;
                // 计算所有的IOU  GIOU CIOU DIOU derivative
                ...
                int obj_index = entry_index(l, b, mask_n*l.w*l.h + j*l.w + i, 5);
            ...
            for (n = 0; n < l.total; ++n) 
            {
                ...
                if (mask_n >= 0 && n != best_n && l.iou_thresh < 1.0f) 
                {
                    ...
                    if (iou > l.iou_thresh) 
                    {
                        ...
                        int class_index = entry_index(l, b, mask_n*l.w*l.h + j*l.w + i, 6);
                        delta_yolo_class(l.output, l.delta, class_index, class_id, l.classes, l.w*l.h, &avg_cat, l.focal_loss, l.label_smooth_eps, l.classes_multipliers, l.cls_normalizer);
        ...
        for (j = 0; j < l.h; ++j) 
        {
            for (i = 0; i < l.w; ++i) 
            {
                for (n = 0; n < l.n; ++n) 
                {
                    ...
                    int class_index = entry_index(l, b, n*l.w*l.h + j*l.w + i, 6);
                    const int stride = l.w*l.h;
        ...
        for (j = 0; j < l.h; ++j) 
        {
            for (i = 0; i < l.w; ++i) 
            {
                for (n = 0; n < l.n; ++n) 
                {
                    ...
                    no_iou_loss_delta[index + 4 * stride] = 0; // 增加的
                }
            }
        }
    }
...
int yolo_num_detections(layer l, float thresh)
{
    ...
    for(n = 0; n < l.n; ++n){
        for (i = 0; i < l.w*l.h; ++i) {
            int obj_index  = entry_index(l, 0, n*l.w*l.h + i, 5);
...
int get_yolo_detections(layer l, int w, int h, int netw, int neth, float thresh, int *map, int relative, detection *dets, int letter)
{
    ...
    for (i = 0; i < l.w*l.h; ++i)
    {
        ...
        for(n = 0; n < l.n; ++n)
        {
            ...
            // printf("%f %f\n",objectness,thresh);
            // if(objectness>temp)temp=objectness;
...
int get_yolo_detections_batch(layer l, int w, int h, int netw, int neth, float thresh, int *map, int relative, detection *dets, int letter, int batch)
{
    ...
    for (i = 0; i < l.w*l.h; ++i){
        ...
        int col = i % l.w;
        for(n = 0; n < l.n; ++n){
            int obj_index  = entry_index(l, batch, n*l.w*l.h + i, 5);
            float objectness = predictions[obj_index];
            //if(objectness <= thresh) continue;    // incorrect behavior for Nan values
            if (objectness > thresh) {
                //printf("\n objectness = %f, thresh = %f, i = %d, n = %d \n", objectness, thresh, i, n);
