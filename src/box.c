...
dbox derivative(box a, box b)
{
    ...
    d.dtheta = a.theta<b.theta?1.0:-1.0;
    return d;
}
...
float box_ciou(box a, box b)
{
    ...
    float rad_a = a.theta/57.29,rad_b=b.theta/57.29;
    float theta=(rad_a-rad_b)*(rad_a-rad_b);
    ...
    printf("  c: %f, u: %f, riou_term: %f\n", c, u, ciou_term);
#endif
    printf("*********** ciou: %f,  theta loss: %f\n", iou-ciou_term,theta);
    return iou - ciou_term+theta;
}
...
dxrep dx_box_iou(box pred, box truth, IOU_LOSS iou_loss) 
{
    ...
    ddx.dtheta = (truth.theta/57.29-pred.theta);
    printf("dtheta: %f\n",ddx.dtheta);
    // ddx.dtheta = (1.57-pred.theta);
