...
box_label *read_boxes(char *filename, int *n)
{
    ...
    int id;
    int count = 0;
    while(fscanf(file, "%d %f %f %f %f %f", &id, &x, &y, &w, &h,&theta) == 6)
    {
...
data load_data_region(int n, char **paths, int m, int w, int h, int size, int classes, float jitter, float hue, float saturation, float exposure)
{
    ...
    printf("during training: load_data_region(data.c 786)\n");
    free(random_paths);
    return d;
}
...
void blend_truth(float *new_truth, int boxes, int truth_size, float *old_truth)
{
    ...
    printf("\n blend_truth(data.c 937)  was %d bboxes, now %d bboxes \n", count_new_truth, t);
}
    ...
    int net_w, int net_h, int mosaic_bound)
{
    ...
    // printf("\n blend_truth_mosaic(data.c 1089) was %d bboxes, now %d bboxes \n", count_new_truth, t);
}
