...
// ====================================================================
    ...
    image mat_to_image_new(cv::Mat mat);
...
extern "C" image DrawLine_New(image a, int x1, int y1, int x2,int y2,float r,float g,float b)
{
    image out;
    cv::Point st(x1,y1);
    cv::Point ed(x2,y2);
    cv:: Mat src = image_to_mat(a);
    if (src.channels() == 3) cv::cvtColor(src, src, cv::COLOR_RGB2BGR);
    // cv::imshow("src",src);
    // printf("r: %f g: %f b %f\n",);
    r *= 255;
    g *= 255;
    b *= 255;
    cv::line(src,st,ed,cv::Scalar(r,g,b),2,8);
    // cv::imshow("linesrc",src);
    
    out = mat_to_image_new(src);
    // show_image(out,"out");
    // cv::waitKey(0);
    return out;
}
...
extern "C" image mat_to_image_new(cv::Mat mat)
{
    int w = mat.cols;
    int h = mat.rows;
    int c = mat.channels();
...
extern "C" cap_cv* get_capture_webcam(int index)
{
    ...
    try {
        ...
        // cap->set(cv::CV_CAP_PROP_FRAME_WIDTH, 608);
        // cap->set(cv::CV_CAP_PROP_FRAME_HEIGHT, 608);
        // cv::resize(*cap,*cap,cv::Size(608,608),0,0,cv::INTER_CUBIC);
    }
    catch (...) {
...
extern "C" image get_image_from_stream_cpp(cap_cv *cap)
{
    ...
    if (once) 
    {
        ...
        // cv::resize(*src,*src,cv::Size(608,608),cv::INTER_CUBIC);
...
extern "C" void draw_box_new_demo(mat_cv* mat, float x, float y,float w,float h,float theta, float r, float g, float b)
{
    cv::Mat *show_img = (cv::Mat*)mat;
    int x1,y1,x2,y2,x3,y3,x4,y4;
    int ori_x1,ori_y1,ori_x2,ori_y2,ori_x3,ori_y3,ori_x4,ori_y4;
    cv::Point st(x1,y1);
    cv::Point ed(x2,y2);
    w = w * show_img->cols;
    h = h * show_img->rows;
    ...
    ori_x4 = 0 + w/2;
    ori_y4 = 0 - h/2;
    float cost = cos(1.57+theta);
    float sint = sin(1.57+theta);
    x1 = ori_x1 * cost - ori_y1*sint + x;
    y1 = ori_x1 * sint + ori_y1*cost + y;

    x2 = ori_x2 * cost - ori_y2*sint + x;
    y2 = ori_x2 * sint + ori_y2*cost + y;

    x3 = ori_x3 * cost - ori_y3*sint + x;
    y3 = ori_x3 * sint + ori_y3*cost + y;

    x4 = ori_x4 * cost - ori_y4*sint + x;
    y4 = ori_x4 * sint + ori_y4*cost + y;
    if(x1<0)x1=0;
    if(x2<0)x2=0;
    if(x3<0)x3=0;
    if(x4<0)x4=0;
    ...
    if(y3<0)y3=0;
    if(y4<0)y4=0;
    // printf("%f %f %f %f %f %f %f\n",x,y,w,h,cos(theta),sin(theta),theta);
    ...
    cv::line(*show_img,st,ed,cv::Scalar(r,g,b),2,8);
    ...
    try 
    {
        ...
        for (i = 0; i < num; ++i) {
            ...
            if (class_id >= 0) 
            {
                ...
                b.y = (b.y < 1) ? b.y : 1;
                //printf("%f %f %f %f\n", b.x, b.y, b.w, b.h);
                draw_box_new_demo(mat, b.x, b.y,b.w,b.h,b.theta, rgb[0], rgb[1], rgb[2]);
